...
function Form() {
  const { topSentence, bottomSentence, dispatch } = useMeme();
  return (
    <form className="w-[70%] flex flex-col gap-[1.5rem] justify-between items-center  mt-[3rem]">
      <input
        ...
        onChange={(e) =>
          dispatch({ type: "setTopSentence", payload: e.target.value })
        }
      ...
      <input
        ...
        onChange={(e) =>
          dispatch({ type: "setBottomSentence", payload: e.target.value })
        }
        className="block input input-bordered w-full max-w-md input-warning text-1.6rem]"
      />
    </form>
